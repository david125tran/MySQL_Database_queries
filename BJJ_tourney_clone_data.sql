...
SELECT 
	competitors.full_name AS winner,
    results.submission,
    results.loser AS loser
FROM competitors 
LEFT JOIN results
...
SELECT
	submissions.submission AS 'Submissions Not Used at Tourney'
FROM submissions
LEFT JOIN results
	ON submissions.id = results.submission
...
SELECT 
	competitors.full_name AS 'Last Place in Weighted Bracket'
FROM competitors 
LEFT JOIN results
	on results.winner = competitors.id
...
SELECT 
	competitors.full_name AS 'last place competitors'
FROM competitors
LEFT JOIN results
	on	competitors.id = results.winner
